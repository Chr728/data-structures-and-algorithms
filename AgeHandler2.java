...
public class AgeHandler2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nNow we will run displaySeniorsIncreasingOrder: ");
		displaySeniorsIncreasingOrder(pName, pDOB, 5); 
	...
	public static void displaySeniorsIncreasingOrder(String[] pName, String[] pDOB, int n) {
		...
		displaySeniorsIncreasingOrder(leftName, leftDate, n);
		displaySeniorsIncreasingOrder(rightName, rightDate, n);
		
		//call mergeSeniorParticipantsForDisplay on every part of the created tree
		mergeSeniorParticipantsForDisplay(pName, pDOB, leftName, leftDate, rightName, rightDate, mid, length-mid); //Goes through list ordering seniors
		
		//Check if seniors are done being ordered
		boolean done;
		int counter = 0;
		...
		done = (counter == n) ? true : false; //This will only ever evaluate to true if the seniors part of the array is finished ordering itself.
	...
	//Works with displaySeniorsIncreasingOrder to reorder the passed arrays
	public static void mergeSeniorParticipantsForDisplay(
