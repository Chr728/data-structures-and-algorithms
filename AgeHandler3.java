...
public class AgeHandler3 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nNow we will run displayNonSeniorsIncreasingOrder: ");
		displayNonSeniorsIncreasingOrder(pName, pDOB, 6, pName.length);
	...
	public static void displayNonSeniorsIncreasingOrder(String[] pName, String[] pDOB, int numJunior, int n) {
		...
		//call mergeNonSeniorParticipantsForDisplay on every part of the created tree
		mergeNonSeniorParticipantsForDisplay(pName, pDOB, leftName, leftDate, rightName, rightDate, mid, length-mid); //Goes through list ordering seniors
		
		//Print non seniors if all recursion is finished
		if (length == n) {
			for (int i = length - 1; i >= numJunior-1; i--) {
	...
	//Works with displayNonSeniorsIncreasingOrder to reorder the passed arrays
	public static void mergeNonSeniorParticipantsForDisplay(
